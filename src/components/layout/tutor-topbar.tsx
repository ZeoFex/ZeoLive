...
import {
  ...
  pruneStaleReadIds,
} from "@/lib/tutor-notifications-storage";
...
export function TutorTopbar({ title }: TutorTopbarProps) {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/tutor/search?q=${encodeURIComponent(query.trim())}`
        );
        const data = await res.json();
        if (res.ok) {
      ...
      <div className="flex flex-1 items-center justify-end gap-2 sm:gap-3">
        <div ref={searchRef} className="relative w-full max-w-xs sm:max-w-sm lg:max-w-md">
          <form onSubmit={handleSearchSubmit}>
            ...
            <Input
              ...
              className="tutor-search w-full"
              aria-label="Search tutor portal"
              value={query}
              onChange={(e) => {
        ...
        <div ref={notifRef} className="relative">
          <Button
            ...
            variant="outline"
            size="icon"
            className="tutor-outline-btn relative h-10 w-10 rounded-xl"
